...
def getQuery(data, match, typeOfhalf):
    ...
    Paris Saint Germain vs Olympiakos CFP
    [u'3 - 2', u'201', u'Any Other Score', u'35', u'2 - 3', u'201']
    """
    queries = []
    for i in range(0, len(data), 2):
        if data[i].find('Any') == -1:
            ...
            try:
                queries.append("INSERT INTO {0} ([home-numberofgoals], [away-numberofgoals], [win-multiplier], [match-name], [update-time]) VALUES ({1}, {2}, {3}, '{4}', getdate())".format(typeOfhalf, home, away, data[i+1], match))
            except:
                pass
    return queries
...
for argv in sys.argv[1:]:
    ...
    for data in tbody:
        ...
        for i in trs[1:]:
            queries += getQuery(dataNormalize(i), match, typeOfhalf)
    doInsertDB(queries)
